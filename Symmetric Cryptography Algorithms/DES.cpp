...
vector<bitset<48>>KeyGeneration(bitset<56> key){
    ...
    bitset<56>combi=Combine(lefthalf,righthalf);
...
bitset<64> Encrypt(bitset<64>plaintext, vector<bitset<48>>keys){
    ...
    bitset<64>final = finalbox(combinedbit);
    // cout<<final;
    return final;
...
int main(){
    ...
    //input
    // cout<<"Enter the binary string for encryption"<<endl;
    
    string x="0111000001101100011000010110100101101110011101000110010101111000";
    // cin>>x;
    bitset<64>plainText (x);
    
    
    // cout<<"Enter the key for encryption"<<endl;
    // cin>>x;
    x="0110110101111001011100110110010101100011011100100110010101110100";
    bitset<64>key(x);

    bitset<56>key1=parityDrop(key);
    
    clock_t start, end;
    double cpu_time_used;
    start = clock();
    vector<bitset<48>> finalKeys;
    finalKeys=KeyGeneration(key1);

    bitset<64>encrypted=Encrypt(plainText,finalKeys);
    cout<<encrypted<<endl;
    bitset<64>decrypted=Decrypt(encrypted,finalKeys);
    cout<<decrypted<<endl;
    end = clock();
    cpu_time_used = static_cast<double>(end - start) / CLOCKS_PER_SEC;
    cout << "Time consumed by insertion sort is " << cpu_time_used << endl;
